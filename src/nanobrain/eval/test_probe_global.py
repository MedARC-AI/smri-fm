...
def test_ridge_head_recovers_linear_signal():
    ...
    oofs = _repeated_oof(X, y, _fit_ridge, n_splits=5, n_repeats=2, seed=0, stratified=False)
    scores = _summarize(y, oofs, REG_METRICS, n_boot=2000, seed=0)
    assert set(scores) == {
...
def test_logistic_head_separable():
    ...
    oofs = _repeated_oof(X, y, _fit_logistic, n_splits=5, n_repeats=2, seed=0, stratified=True)
    scores = _summarize(y, oofs, CLS_METRICS, n_boot=2000, seed=0)
...
def test_logistic_head_positive_column_with_minority_class():
    ...
    oofs = _repeated_oof(X, y, _fit_logistic, n_splits=5, n_repeats=2, seed=0, stratified=True)
    assert _summarize(y, oofs, CLS_METRICS, n_boot=200, seed=0)["auroc"] > 0.9
...
def test_summarize_deterministic():
    ...
    oofs = _repeated_oof(X, y, _fit_ridge, 5, 2, seed=7, stratified=False)
    assert _summarize(y, oofs, REG_METRICS, 2000, 7) == _summarize(y, oofs, REG_METRICS, 2000, 7)
...
def test_extract_features():
    ...
    X = _extract_features(model, dataset, "image", CPU)
    assert X.shape == (6, 64)
...
def test_read_targets_remaps():
    ...
    y = _read_targets(dataset, "dx", target_map={"CN": 0, "AD": 1})
    assert list(y) == [0, 1, 0]
...
def test_reg_probe_matches_its_parts():
    ...
    X = _extract_features(model, dataset, task.image_col, CPU)
    y = _read_targets(dataset, task.target_col).astype(float)
    oofs = _repeated_oof(X, y, _fit_ridge, 3, 1, seed=0, stratified=False)
    assert scores == _summarize(y, oofs, REG_METRICS, 50, 0)

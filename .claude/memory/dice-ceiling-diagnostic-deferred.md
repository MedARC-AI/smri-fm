...
description: A per-(model, task) Dice ceiling would separate "too coarse" from "uninformative" in the seg probe — deferred 2026-08-06 as not strictly necessary; includes the exact formula and the ratio trap.
metadata:
  type: project
---

Under the patch-point-cloud contract ([[seg-probe-patch-contract]]) the seg probe assigns each
...
(model, task). Deferred 2026-08-06: not strictly necessary, and raw Dice is comparable without it.
